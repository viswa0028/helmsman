...
function banner(color, icon, label) {
...
// ── agent loop ──────────────────────────────────────
async function runAgent(systemPrompt, agentName, color, userPrompt) {
    const messages = [
    ...
    while (iterations < MAX_ITERATIONS) {
        ...
        for (const toolCall of message.tool_calls) {
            ...
            console.log(`${C.dim}  ↳ calling ${C.cyan}${fnName}${C.reset}${C.dim}(${JSON.stringify(fnArgs).slice(0, 80)})${C.reset}`);
            try {
...
async function main() {
    ...
    await runAgent(FINOPS_SYSTEM, 'FinOps', C.yellow, `Analyse the current cluster state in the "shop" namespace.
...
Provide cost savings estimates in INR/hr.`);
    ...
    if (actions.length > 0) {
        ...
        await runAgent(GUARDIAN_SYSTEM, 'Guardian', C.green, `Review the current state of the "shop" namespace after the FinOps agent has acted.
...
${JSON.stringify(actions, null, 2)}`);
    ...
    if (allActions.length === 0) {
        ...
        await runAgent(SUMMARY_SYSTEM, 'Summary', C.blue, `Generate a change record from this remediation cycle.
...
Include executed changes, blocked changes with veto reasons, and net cost impact.`);
    ...
    console.log(`${C.dim}   Total actions: ${allActions.length} ` +
